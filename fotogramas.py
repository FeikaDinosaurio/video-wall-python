...
def reproducir_video_local(frames, fps=30, global_frame_index=None, global_frame_index_lock=None):
...
def handle_client(client_socket, frames, fps, global_frame_index, global_frame_index_lock):
    try:
        enviar_comandos_sincronizacion(client_socket, frames, fps, global_frame_index, global_frame_index_lock)
    except OSError as e:
        if e.winerror == 10038:
            print(f"El socket del cliente ya está cerrado.")
    finally:
        client_socket.close()
